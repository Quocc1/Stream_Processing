...
def get_id_list(type: str) -> list:
    ...
    id_list = df[column_name].to_list()
    return id_list
...
def generate_checkout_event(user_id: int, product_id: int) -> dict:
    ...
    )  # Random value between [1.00, 10.00] USD
    discount = round(
        random.uniform(0.05, 0.5), 2
    )  # Random value between [5, 50] percentage
...
def generate_click_datastream() -> None:
    ...
    while random.randint(1, 100) <= 50:
        checkout_event = generate_checkout_event(
            click_event["user_id"], get_random_product_id()
        )
