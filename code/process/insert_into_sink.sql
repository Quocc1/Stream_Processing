...
FROM (
        SELECT co.checkout_id,
            ...
            p.category,
            (
                p.price * co.quantity * co.discount - co.shipping_cost
            ) * p.commission_rate AS profit,
            cl.source,
        ...
        FROM checkouts AS co
            ...
            AND co.checkout_timestamp BETWEEN cl.click_timestamp AND cl.click_timestamp + INTERVAL '1' HOUR
    )
WHERE rn = 1;